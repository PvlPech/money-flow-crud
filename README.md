# money-flow-crud
## Create/initialize the worker
https://developers.cloudflare.com/workers/get-started/guide/
## D1 DB creation
https://developers.cloudflare.com/d1/get-started/
## Initialize drizzle orm usage to comunicate with D1 (cloudflare DB)
https://github.com/drizzle-team/drizzle-orm/tree/main/examples/cloudflare-d1
### Drizzle orm usage (itty-router)
https://github.com/drizzle-team/drizzle-orm/blob/main/examples/cloudflare-d1/src/index.ts
### Itty-router with support of openai
https://cloudflare.github.io/itty-router-openapi/user-guide/first-steps/

https://github.com/cloudflare/itty-router-openapi/blob/main/TUTORIAL.md
### Drizzle schema implementation for D1 (cloudflare)
https://dev.to/franciscomendes10866/getting-started-with-drizzle-orm-a-beginners-tutorial-4782

https://orm.drizzle.team/docs/migrations/#declare-your-schema

https://github.com/drizzle-team/drizzle-orm/blob/main/examples/cloudflare-d1/src/schema.ts

https://orm.drizzle.team/docs/indexes-constraints/#foreign-key

https://orm.drizzle.team/docs/rqb/#one-to-many
### itty-router-extras
https://github.com/kwhitley/itty-router-extras